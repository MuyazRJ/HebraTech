...
const Landing = ({ doneLoading }: { doneLoading: boolean}) => {
    ...
    return ( 
        <div className="relative">
            <motion.div key="landing" className="w-full bg-main-black overflow-hidden" initial={{scaleY:0}} animate={{scaleY:1}} transition={{duration:1, ease:[0.99, 0, 0.17, 1], delay: 0.3}}>
            ...
            <motion.div key="pre" className={`w-screen h-screen bg-red-200 origin-top -z-[11] ${doneLoading ? 'hidden': 'fixed'}`} initial={{scaleY:0}} animate={{scaleY:1}} transition={{duration:1, ease:[0.99, 0, 0.17, 1]}}/>
        </div>
     );
}
